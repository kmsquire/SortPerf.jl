...
randfns = Dict(Int => randint,
               Int32 => x->round(Int32,randint(x)),
               Int64 => x->round(Int64,randint(x)),
               Int128 => x->round(Int128,randint(x)),
               Float32 => x->round(Float32,rand(x)),
               Float64 => rand,
               String => randstr)
...
# DataFrame labels
labels = [symbol("test_type"), symbol("sort_alg"), symbol("log_size"), symbol("size"), symbol("*sort"),
          symbol("\\sort"), symbol("/sort"), symbol("3sort"), symbol("+sort"), symbol("~sort"), symbol("=sort"), symbol("!sort")]

# Corresponding descriptions
sort_descr = Dict( '*' => "random",
                   '\\' => "reversed",
                   '/' => "sorted",
                   '3' => "3 exchanges",
                   '+' => "10 appended",
...
function _sortperf(alg::Algorithm, origdata::Vector, order::Ordering=Forward; replicates=3, skip_median_killer=false)
    ...
    rec = Dict(symbol("test_type") => string(eltype(origdata)),
               symbol("sort_alg") => string(alg)[1:end-5],
               symbol("log_size") => isinteger(logn) ? round(Int, logn) : logn,
               symbol("size") => length(origdata))

    if alg==RadixSort && !isbits(origdata[1])
        println("Skipping RadixSort for non bitstype $(eltype(origdata))")
        reptimes = Dict{Symbol, Float64}()
    ...
    for rep = 1:replicates
        ...
        if !issorted(data, order)
            reptimes[symbol("*sort")] = @elapsed sort!(data, alg, order)
        ...
        gc()
        reptimes[symbol("3sort")] = @elapsed sort!(data, alg, order)
        @assert issorted(data, order)
        ...
        gc()
        reptimes[symbol("~sort")] = @elapsed sort!(data4, alg, order)
        @assert issorted(data, order)

        ## All values equal
        data1 = data[fill(rand(1:n), n)]
        gc()
        reptimes[symbol("=sort")] = @elapsed sort!(data1, alg, order)
        ...
        if skip_median_killer || alg==QuickSort && !(eltype(data) <: Integer) && length(data) >= 2^18
            ...
            gc()
            reptimes[symbol("!sort")] = @elapsed sort!(qdata, alg, order)
...
function _sortperf(alg::Algorithm, data::Vector, log2range::Range, order::Ordering=Forward; named...)
    lg2range = filter(x -> 2^x <= length(data), collect(log2range))
    times = Dict[]
...
function _sortperf(alg::Algorithm, T::Type, log2range::Range, args...; named...)
    println("Testing $T...")
...
function sort_plots(df, base_sort, cols = ["*sort_median", "\\sort_median", "/sort_median", "3sort_median",
    ...
    colors = Dict([symbol(alg) for alg in sort_algs], Uint32[convert(RGB24, r) for r in dc])
